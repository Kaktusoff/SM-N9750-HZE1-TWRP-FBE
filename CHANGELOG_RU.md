...
Обновлены установщик и документация; бинарные образы не изменились относительно
v1.0.0.

- Добавлен защищённый режим `--flash-no-wipe` для проверенной пары
  BOOT+RECOVERY на уже разблокированном устройстве с точной HZE1.
- Добавлен режим `--twrp-only-no-wipe`, прошивающий RECOVERY без изменения BOOT
  и USERDATA.
- Явно указано, что первая разблокировка загрузчика Samsung всегда стирает
  данные, а стоковый BOOT способен восстановить стоковый recovery после
  установки только RECOVERY.
